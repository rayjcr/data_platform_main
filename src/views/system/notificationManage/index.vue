                ...
                <a-card style="border-radius:2px;" :bodyStyle="{padding:'24px 24px 5px 24px'}" :bordered="false" v-if="!visible">
                    <a-form class="searchform">
                        <a-form-item :colon="false" class="main_margin_bottom_15">
                            <a-input style="width:250px; margin-right: 10px" allowClear v-model="searchValue" placeholder="请输入标题" />
                    ...
                    >
                        ...
                        <div slot="omit" slot-scope="text">
                            <div class="omitBox">
                                <div class="omitBox-item" :title="text">{{text}}</div>
                            </div>
                        </div>
                 ...
                 <!-- 新增编辑 -->
                <add ref="add" v-if="visible" :info="info" :type="type" @exit="exit" @save="save"></add>
            </div>
        </div>
...
import { getNotificationlist, saveNotification,delNotification,updateStatus} from '@/api/notification.js'
export default {
    // 通知公告管理
    name:'notification',
    components:{
    ...
    methods:{
        ...
        // 保存
        async save(data){
            let res = await saveNotification(data)
        ...
        async del(data) {
            this.$confirm({
                ...
                cancelText: '取消',
                onOk: async() => {
                    let res = await delNotification({noticeId: data.id})
                    if (res.resultCode == 200) {
                        this.$success({
                            centered: true,
                            title: '成功',
       ...
       async send(type,item){
            this.$confirm({
                centered: true,
                title: '确认提示',
                content: '您确定要' + (type == 0 ? '取消发布' : '发布' ) + '选中的通知吗?',
                okText: '确认',
                cancelText: '取消',
                onOk: async() => {
                    let res = await updateStatus({noticeId: item.id,status: type})
                    if (res.resultCode == 200) {
                        this.$success({
                            centered: true,
                            title: '成功',
